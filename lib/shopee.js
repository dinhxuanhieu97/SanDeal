import crypto from 'node:crypto';

// Các domain Shopee hợp lệ được chấp nhận
export const SHOPEE_HOSTS = ['shopee.vn', 'shopee.com', 's.shopee.vn', 'shp.ee'];

/** Kiểm tra một chuỗi có phải link Shopee hợp lệ không */
export function isShopeeUrl(raw) {
  if (typeof raw !== 'string') return false;
  try {
    const u = new URL(raw.trim());
    if (u.protocol !== 'http:' && u.protocol !== 'https:') return false;
    const host = u.hostname.replace(/^www\./, '').toLowerCase();
    return SHOPEE_HOSTS.some((h) => host === h || host.endsWith('.' + h));
  } catch {
    return false;
  }
}

/** DEMO: gắn tham số theo dõi (sub_id / utm) vào link Shopee gốc */
export function buildDemoLink(raw, subId = 'sandeal') {
  const u = new URL(raw.trim());
  u.searchParams.set('af_sub1', subId);
  u.searchParams.set('utm_source', subId);
  u.searchParams.set('utm_medium', 'affiliate');
  return u.toString();
}

/** Chữ ký chuẩn Shopee: SHA256(AppId + Timestamp + Payload + AppSecret) */
export function signRequest({ appId, appSecret, timestamp, payload }) {
  const base = `${appId}${timestamp}${payload}${appSecret}`;
  return crypto.createHash('sha256').update(base).digest('hex');
}

/**
 * PRODUCTION: gọi Shopee Affiliate Open API (GraphQL) tạo shortlink chính thức.
 * fetchImpl & timestamp tách ra tham số để unit test có thể giả lập.
 */
export async function buildOfficialShortLink(
  raw,
  { appId, appSecret, subId = 'sandeal', timestamp, fetchImpl = fetch, timeoutMs = 5000 } = {}
) {
  const ts = timestamp ?? Math.floor(Date.now() / 1000);
  const endpoint = 'https://open-api.affiliate.shopee.vn/graphql';
  const query = `mutation{generateShortLink(input:{originUrl:${JSON.stringify(
    raw
  )},subIds:[${JSON.stringify(subId)}]}){shortLink}}`;
  const payload = JSON.stringify({ query });
  const signature = signRequest({ appId, appSecret, timestamp: ts, payload });

  // Tự huỷ sau timeoutMs để API treo không kéo sập request (nhánh fallback sẽ xử lý)
  const res = await fetchImpl(endpoint, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `SHA256 Credential=${appId}, Timestamp=${ts}, Signature=${signature}`,
    },
    body: payload,
    signal: AbortSignal.timeout(timeoutMs),
  });

  const data = await res.json();
  const link = data?.data?.generateShortLink?.shortLink;
  if (!link) {
    const msg = data?.errors?.[0]?.message || 'Không tạo được shortlink';
    throw new Error(msg);
  }
  return link;
}

/**
 * Tách shopId + itemId từ link sản phẩm Shopee.
 * Hỗ trợ ".../ten-i.SHOPID.ITEMID" và ".../product/SHOPID/ITEMID".
 * Trả null nếu không tách được (vd shortlink s.shopee.vn cần resolve trước).
 */
export function parseShopeeIds(raw) {
  if (typeof raw !== 'string') return null;
  try {
    const path = decodeURIComponent(new URL(raw.trim()).pathname);
    let m = path.match(/i\.(\d+)\.(\d+)/);
    if (m) return { shopId: Number(m[1]), itemId: Number(m[2]) };
    m = path.match(/\/product\/(\d+)\/(\d+)/);
    if (m) return { shopId: Number(m[1]), itemId: Number(m[2]) };
    return null;
  } catch {
    return null;
  }
}

/**
 * Gọi Shopee Affiliate Open API (productOfferV2) lấy thông tin sản phẩm thật:
 * tên, giá, ảnh, hoa hồng, offerLink. Cần credentials. fetchImpl/timestamp tách để test.
 */
export async function queryProductOffer(
  { itemId, shopId, appId, appSecret, timestamp, fetchImpl = fetch, timeoutMs = 5000 } = {}
) {
  const ts = timestamp ?? Math.floor(Date.now() / 1000);
  const endpoint = 'https://open-api.affiliate.shopee.vn/graphql';
  const query =
    `{productOfferV2(itemId:${itemId},shopId:${shopId},limit:1){` +
    `nodes{productName priceMin priceMax imageUrl offerLink productLink commissionRate shopName sales ratingStar}}}`;
  const payload = JSON.stringify({ query });
  const signature = signRequest({ appId, appSecret, timestamp: ts, payload });

  const res = await fetchImpl(endpoint, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `SHA256 Credential=${appId}, Timestamp=${ts}, Signature=${signature}`,
    },
    body: payload,
    signal: AbortSignal.timeout(timeoutMs),
  });

  const data = await res.json();
  const node = data?.data?.productOfferV2?.nodes?.[0];
  if (!node) {
    const msg = data?.errors?.[0]?.message || 'Không tìm thấy sản phẩm cho link này';
    throw new Error(msg);
  }
  return node;
}

/**
 * Lấy danh sách sản phẩm ưu đãi (deal) theo từ khoá qua Shopee Affiliate API.
 * Trả về mảng node (mỗi node đã có offerLink gắn tracking sẵn).
 */
export async function queryHotOffers(
  { keyword, limit = 6, appId, appSecret, timestamp, fetchImpl = fetch, timeoutMs = 8000 } = {}
) {
  const ts = timestamp ?? Math.floor(Date.now() / 1000);
  const endpoint = 'https://open-api.affiliate.shopee.vn/graphql';
  const kw = JSON.stringify(String(keyword || ''));
  const query =
    `{productOfferV2(keyword:${kw},limit:${limit}){` +
    `nodes{itemId shopId productName priceMin priceMax price priceDiscountRate ` +
    `imageUrl offerLink productLink commissionRate shopName sales ratingStar}}}`;
  const payload = JSON.stringify({ query });
  const signature = signRequest({ appId, appSecret, timestamp: ts, payload });

  const res = await fetchImpl(endpoint, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `SHA256 Credential=${appId}, Timestamp=${ts}, Signature=${signature}`,
    },
    body: payload,
    signal: AbortSignal.timeout(timeoutMs),
  });

  const data = await res.json();
  const nodes = data?.data?.productOfferV2?.nodes;
  if (!Array.isArray(nodes)) {
    const msg = data?.errors?.[0]?.message || 'Không lấy được danh sách deal';
    throw new Error(msg);
  }
  return nodes;
}
